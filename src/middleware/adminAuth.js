...
function password() { return String(process.env.ADMIN_PANEL_PASSWORD || ""); }
...
function setSessionCookie(req, res) {
  const attributes = [`${COOKIE_NAME}=${encodeURIComponent(createSession())}`, "Path=/", "HttpOnly",
    "SameSite=Strict", `Max-Age=${sessionSeconds()}`];
...
function matchesPassword(candidate) {
  const configured = Buffer.from(password()); const supplied = Buffer.from(String(candidate || ""));
...
function hasInternalKey(req) {
  ...
  const left = Buffer.from(String(required)); const right = Buffer.from(String(supplied || ""));
  return left.length === right.length && crypto.timingSafeEqual(left, right);
}
...
module.exports = { COOKIE_NAME, createSession, validSession, matchesPassword, hasInternalKey,
  setSessionCookie, clearSessionCookie, requireAdminPage, requireAdminApi };
