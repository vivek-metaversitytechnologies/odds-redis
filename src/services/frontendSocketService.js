...
function validMarketId(value) {
  return (
    typeof value === "string" && value.length > 0 && value.length <= 100 && /^[A-Za-z0-9._-]+$/.test(value)
  );
}
...
function adminSocketAuthorized(socket) {
  ...
  return (
    validSession(socket.request) ||
    (required && socket.handshake.auth?.apiKey === required) ||
...
function attachFrontendSocket(server) {
  io = new Server(server, {
    path: process.env.FRONTEND_SOCKET_PATH || "/socket.io",
    ...
    client.on("subscribe:event", async (eventId, acknowledge = () => {}) => {
      ...
      try {
        const [snapshot, score] = await Promise.all([
          redis.getEventSnapshot(eventId),
          redis.getScore(eventId),
        ]);
  ...
  providerSocket.setScorePublisher((eventId, payload, receivedAtMs) => {
    if (eventId != null)
      io.to(`event:${eventId}`).emit("score", {
        eventId: String(eventId),
        data: payload,
        receivedAt: new Date(receivedAtMs || Date.now()).toISOString(),
      });
  });
  providerSocket.setRawTickPublisher((tick, receivedAtMs) => {
    ...
    io.to(`admin:market:${marketId}`).emit("raw:market", {
      type: "provider.socket.raw",
      marketId,
      timestamp: new Date(receivedAtMs || Date.now()).toISOString(),
      payload: tick,
    });
  });
...
async function closeFrontendSocket() {
  ...
  providerSocket.setScorePublisher(null);
