...
async function upsertEvents(events) {
  ...
  try {
    ...
    const [existingRows] = await connection.query(
      `SELECT eventid FROM t_event WHERE eventid IN (${ids.map(() => "?").join(",")})`, ids,
    );
    ...
    for (const event of events) {
      ...
      await connection.execute(
        `INSERT INTO t_event
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW(), ?, ?, ?, ?, ?, ?, ?, ?)`,
        [event.seriesId, event.sportId, event.eventId, event.eventName, event.openDate,
          true, true, false, false, true, event.inPlay, true, true, true, "1"],
      );
      inserted += 1;
    }
...
async function syncEvents() {
  ...
  try {
    ...
    const received = responses.reduce((total, response) => total
      + (Array.isArray(response?.data) ? response.data.length : Array.isArray(response) ? response.length : 0), 0);
    const events = eventRows(responses); const persisted = await upsertEvents(events);
    const marketDiscovery = await syncMarketDiscovery(events);
    const result = { skipped: false, received, supported: events.length, sportIds,
      ...persisted, marketDiscovery };
    state.lastResult = result; state.lastCompletedAt = new Date().toISOString();
    logger.info("[EventSync] completed", result); return result;
  } catch (error) {
    state.lastError = error.message; state.lastCompletedAt = new Date().toISOString();
    logger.error("[EventSync] failed", { error: error.message }); throw error;
...
function startEventSync() {
  ...
  logger.info("[EventSync] scheduled", { expression, sportIds: [...supportedSportIds()] }); return task;
}

function getEventSyncStatus() { return { ...state, sportIds: [...supportedSportIds()] }; }
