...
function supportedSportIds() {
  return new Set(String(process.env.SPORT_IDS || "1,2,4")
...
function competitionRows(response) {
  ...
  return rows.map((item) => ({
    seriesId: Number(item?.id), seriesName: String(item?.name || "").trim(), sportId: Number(item?.sportId),
...
async function upsertCompetitions(competitions) {
  ...
  try {
    ...
    const [existingRows] = await connection.query(
      `SELECT seriesid FROM t_series WHERE seriesid IN (${ids.map(() => "?").join(",")})`, ids,
    );
...
async function syncCompetitions() {
  ...
  try {
    ...
    const received = Array.isArray(response?.data) ? response.data.length : Array.isArray(response) ? response.length : 0;
    const result = { skipped: false, received, supported: competitions.length,
      sportIds: [...supportedSportIds()], ...persisted };
...
function getCompetitionSyncStatus() { return { ...state, sportIds: [...supportedSportIds()] }; }

module.exports = { supportedSportIds, competitionRows, upsertCompetitions, syncCompetitions,
  startCompetitionSync, getCompetitionSyncStatus };
