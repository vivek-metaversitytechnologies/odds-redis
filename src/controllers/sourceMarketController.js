  ...
  try {
    const [rows] = await getSourcePool().query(
      `SELECT f.id, f.fancyid, f.name, f.oddstype, f.status, f.eventid, f.isactive,
       ...
       ORDER BY f.eventid DESC, f.id DESC`,
      [1, 2, 4],
    );
    res.json({ status: "ok", data: rows, meta: { total: rows.length, sportIds: [1, 2, 4] } });
  } catch (error) {
...
function eventSyncStatus(req, res) {
  res.json({ status: "ok", data: getEventSyncStatus() });
}
...
async function subscribeManual(req, res, next) {
  try {
    const marketIds = [
      ...new Set(
        (Array.isArray(req.body?.marketIds) ? req.body.marketIds : [])
          .map(String)
          .map((id) => id.trim())
          .filter(Boolean),
      ),
    ...
    if (!marketIds.length || marketIds.length > 100) {
      const error = new Error("Provide between 1 and 100 market IDs");
...
async function unsubscribeEvent(req, res, next) {
  try {
    ...
    if (!/^\d+$/.test(eventId) || Number(eventId) <= 0) {
      const error = new Error("A positive numeric event ID is required");
      error.statusCode = 400;
      throw error;
    }
    const [rows] = await getSourcePool().query(
    ...
    if (!marketIds.length) {
      const error = new Error("No markets found for this event");
    ...
    recordMarketSyncActivity("event.unsubscribe.completed", {
      eventId,
      markets: result.unsubscribed.length,
    });
