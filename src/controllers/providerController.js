...
module.exports = {
  ...
  competitions: handler(async (req) => items(await provider.competitions(req.query))
    .filter((item) => same(item.sportId ?? item.sportid, req.query.sportId))),
  events: handler(async (req) => items(await provider.events(req.query))
    .filter((item) => same(item.sportId ?? item.sportid, req.query.sportId))
    .filter((item) => same(item.competitionId ?? item.competitionid ?? item.leagueId, req.query.competitionId))
    .filter((item) => req.query.includeCompleted === "true" || item.gameOver !== true)
    .sort((left, right) => Number(Boolean(right.inPlay)) - Number(Boolean(left.inPlay)) ||
      new Date(left.startTime || 0).getTime() - new Date(right.startTime || 0).getTime())),
  markets: handler(async (req) => {
    const eventIds = new Set((Array.isArray(req.body?.data) ? req.body.data : req.body?.eventIds || [])
      .map(String));
