...
test("line markets refresh prices from the runner endpoint", () => {
  ...
  assert.match(source, /const prices = await seedInitialMarketPrices\(active, \{ refreshLines: false \}\)/);
  assert.match(source, /const prices = await seedInitialMarketPrices\(markets\);/);
...
test("socket game-over cleans up immediately while corrected vendor state can restore an event", () => {
  ...
  assert.match(resultSource, /WHERE m\.isactive=\?/);
