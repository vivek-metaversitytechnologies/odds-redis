...
test("manual subscription requires at least one market ID", async () => {
  const response = await request(createApp()).post("/api/source/subscribe")
    .set("X-Internal-API-Key", process.env.INTERNAL_API_KEY).send({ marketIds: [] }).expect(400);
...
test("event unsubscribe requires a valid event ID", async () => {
  const response = await request(createApp()).post("/api/source/events/not-an-id/unsubscribe")
    .set("X-Internal-API-Key", process.env.INTERNAL_API_KEY).expect(400);
...
test("admin login creates an HttpOnly session that opens the panel", async () => {
  ...
  const login = await agent.post("/admin/auth/login")
    .send({ password: process.env.ADMIN_PANEL_PASSWORD }).expect(200);
